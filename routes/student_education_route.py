...
from schemas.task_schema import TaskHistoryCreate, TaskHistoryResponse
...
from services.student_family_service import (
    add_family_member_service,
    hard_delete_family_member_service,
    list_family_members_service,
    update_family_member_service,
)
...
def add_full_student_profile(
    ...
    return {"message": "Student profile updated successfully", "data": results}
...
@router.get(
    "/{student_id}/family-members", response_model=List[StudentFamilyMemberResponse]
)
...
def get_internships(
    ...
    return get_internship_list_service(
        db=db, category_id=category_id, work_type_id=work_type_id
    )
...
def add_task_rating_api(
    ...
    return add_task_rating(db=db, payload=payload, created_by=user_id)
